...
\begin{exercise}\label{ex6.1}
	\begin{enumerate}[label=(\roman*),ref=\theexercise(\roman*)]\crefalias{enumi}{exercise}
		\item Let $x\in\operatorname{ker}(u)$. Since $u^n$ surjective, there is a $y\in M$ such that $u^n(y)=x$. But $u^{n+1}(y)=u(x)=0$, means that $y\in\operatorname{ker}\left(u^{n+1}\right)=\operatorname{ker}\left(u^n\right)$.
...
\begin{exercise}\label{ex6.11}
	By \Cref{ex6.7}, $V(\mathfrak{b})$ is a finite union of $V(\mathfrak{q}_i)$, where $\mathfrak{q}_i$ minimal and containing $\mathfrak{b}$. Then use \hyperref[ex5.10]{Exercise 5.10(c)}.
\end{exercise}

\begin{exercise}\label{ex6.12}
	The converse isn't true. The counter example is an infinite product $\prod\mathbb{Z}/2\mathbb{Z}$.
\end{exercise}