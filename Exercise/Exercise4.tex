...
\begin{exercise}\label{ex4.8}
	$k\left[x_1,\cdots,x_n\right]/\mathfrak{p_i}=k\left[x_{i+1},\cdots,x_n\right]$ integer domain, so $\mathfrak{p}_i$ prime. Note that $\mathfrak{p}_i^n$ is the set of all homogeneous polynomials in $k\left[x_1,\cdots,x_i\right]$ of degree $n$. Use the fact that $fg$ is homogeneous $\Rightarrow$ $f$ and $g$ are homogeneous.
\end{exercise}
...
\begin{exercise}\label{ex4.9}
	\begin{enumerate}[label=(\roman*),ref=\theexercise(\roman*)]\crefalias{enumi}{exercise}
		...
		\item Let $(0:a/s)\subseteq S^{-1}\mathfrak{p}$. If $(0:a/s)^c\subseteq\mathfrak{p}^\prime\subsetneq\mathfrak{p}$, use $(0:a/s)^{ce}=(0:a/s)$ derived from \Cref{3.11.1}. Hence $D\left(S^{-1}A\right)$ is a subset of $D(A)$. $S^{-1}\mathfrak{p}$ is a prime ideal if $S$ doesn't meet $\mathfrak{p}$.
