...
\begin{exercise}\label{ex4.8}
	$k\left[x_1,\cdots,x_n\right]/\mathfrak{p_i}=k\left[x_{i+1},\cdots,x_n\right]$ integer domain, so $\mathfrak{p}_i$ prime. Note that $\mathfrak{p}_i^n$ is the set of all homogeneous polynomials in $k\left[x_1,\cdots,x_i\right]$ of degree $\geqslant n$. Use the fact that $fg$ is homogeneous $\Rightarrow$ $f$ and $g$ are homogeneous.
\end{exercise}
