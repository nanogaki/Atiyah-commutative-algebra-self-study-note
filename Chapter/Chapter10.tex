...
\begin{proposition}{10.13}
	If $M$ is finitely-generated $A$-module, $\hat{A}\otimes_AM\rightarrow\hat{M}$ is surjective. If $A$ is Noetherian, then $\hat{A}\otimes_AM\rightarrow\hat{M}$ is an isomorphism.
\end{proposition}
...
\begin{proposition}{10.15}
	...
	\begin{enumerate}[label=\textup{(\roman*)}]
		\item $\hat{\mathfrak{a}}=\hat{A}\mathfrak{a}\cong\hat{A}\otimes_A\mathfrak{a}$;
		\item $\widehat{\left(\mathfrak{a}^n\right)}=\left(\hat{\mathfrak{a}}\right)^n$;
		\item $\hat{\mathfrak{a}}^n/\hat{\mathfrak{a}}^{n+1}\cong\mathfrak{a}^n/\mathfrak{a}^{n+1}$;
		\item $\hat{\mathfrak{a}}$ is contained in the Jacobson radical of $\hat{A}$.
	\end{enumerate}
\end{proposition}
...
\begin{proposition}{10.17}[Kull's theorem]
	Let $A$ be a Noetherian ring, $\mathfrak{a}$ an ideal, $M$ a finitely-generated $A$-module and $\hat{M}$ the $\mathfrak{a}$-adic completion of $M$. Then the kernel $E=\bigcap_{n=1}^\infty\mathfrak{a}^nM$ of $M\rightarrow\hat{M}$ consists of those $x\in M$ annihilated by some element of $1+\mathfrak{a}$.
\end{proposition}
